...
//
void OneWire_BusInputDirection(OneWire_TypeDef *onewire)
{
...
void OneWire_BusOutputDirection(OneWire_TypeDef *onewire)
{
   GPIO_InitTypeDef GPIO_InitStruct;
...
//
void OneWire_OutputLow(OneWire_TypeDef *onewire)
{
   onewire->GPIOx->BSRR = onewire->GPIO_Pin << 16; // Reset the 1-Wire pin
}

void OneWire_OutputHigh(OneWire_TypeDef *onewire)
{
...
//	1 - Error
//
uint8_t OneWire_Reset(OneWire_TypeDef *onewire)
{
   uint8_t i;
...
//
void OneWire_WriteBit(OneWire_TypeDef *onewire, uint8_t bit)
{
   if(bit) // Send '1',
   {
...
uint8_t OneWire_ReadBit(OneWire_TypeDef *onewire)
{
...
void OneWire_WriteByte(OneWire_TypeDef *onewire, uint8_t byte)
{
   uint8_t i = 8;
...
uint8_t OneWire_ReadByte(OneWire_TypeDef *onewire)
{
   uint8_t i = 8, byte = 0;
...
//
void OneWire_ResetSearch(OneWire_TypeDef *onewire)
{
...
uint8_t OneWire_Search(OneWire_TypeDef *onewire, uint8_t command)
{
...
//
uint8_t OneWire_First(OneWire_TypeDef *onewire)
{
...
//
uint8_t OneWire_Next(OneWire_TypeDef *onewire)
{
...
//
void OneWire_Select(OneWire_TypeDef *onewire, uint8_t *addr)
{
   uint8_t i;
...
//
void OneWire_SelectWithPointer(OneWire_TypeDef *onewire, uint8_t *ROM)
{
   uint8_t i;
   OneWire_WriteByte(onewire, ONEWIRE_CMD_MATCHROM); // Match ROM command
...
//
void OneWire_GetFullROM(OneWire_TypeDef *onewire, uint8_t *firstIndex)
{
   uint8_t i;
   for(i = 0; i < 8; i++)
   {
